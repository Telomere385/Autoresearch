...
if(len(argv)) == 2:
    if argv[1] == 'play':
        ...
        # Q_old = np.load('Q_last_lr_0.6.npy')  # 3.3.2-1
    ...
    while True:
        ...
        if crashTest[0]:
            ...
            if counter % 100 == 0:
                ...
                np.save('prefill_heuristic.npy', np.array(ave_rewards, dtype=float))

                rewsum = 0
                highscore = 0
                totscore = 0
                
                np.save('Q_last_pre_heuristic.npy', Q)
                np.save('rewards_pre_heuristic.npy', np.array(episode_rewards, dtype=float)) #Plot #save_rewards
